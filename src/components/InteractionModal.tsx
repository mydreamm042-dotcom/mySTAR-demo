  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (res?.error) {
        ...
        const msgs: Record<Mode, string> = { select: '', heart: '💖 하트를 보냈어요!', warning: '🤫 조용히 전달했어요', star: `⭐ ${starValue}점 투표 완료!` }
    ...
    <div style={{ position: 'fixed', inset: 0, zIndex: 50, display: 'flex', flexDirection: 'column', background: 'rgba(0,0,0,0.95)', backdropFilter: 'blur(16px)' }}>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', padding: '56px 24px 24px', overflow: 'hidden' }}>
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 32 }}>
          <div>
            <span className="badge" style={{ background: 'rgba(255,107,107,0.15)', color: 'var(--accent)', marginBottom: 8, display: 'inline-flex' }}>ROUND {round}</span>
            <h2 style={{ fontSize: 26, fontWeight: 800 }}>지금 표현해볼까요?</h2>
          </div>
          <button onClick={onClose}
            style={{ width: 40, height: 40, borderRadius: 12, background: 'var(--card)', border: '1px solid var(--border)', color: 'var(--muted2)', fontSize: 18, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}>✕</button>
        </div>
        ...
        {mode === 'select' && (
          <div className="animate-fade-in" style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {[
              ...
              { m: 'star' as Mode, emoji: '⭐', label: '분위기 별점', desc: '지금 이 자리, 몇 점짜리?', color: 'var(--purple-light)', bg: 'rgba(124,92,191,0.1)', border: 'rgba(124,92,191,0.25)' },
        ...
        {(mode === 'heart' || mode === 'warning') && (
          <div className="animate-fade-in" style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
            <button onClick={() => { setMode('select'); setSelectedId(null) }}
              style={{ background: 'none', border: 'none', color: 'var(--muted2)', fontSize: 14, cursor: 'pointer', marginBottom: 20, textAlign: 'left' }}>← 뒤로</button>
            <p style={{ fontSize: 20, fontWeight: 800, marginBottom: 4 }}>{mode === 'heart' ? '💖 하트 보낼 사람' : '🤫 시그널 보낼 사람'}</p>
            <p style={{ fontSize: 13, color: 'var(--muted2)', marginBottom: 20 }}>{mode === 'heart' ? '라운드당 1명에게만' : '익명으로 전달돼요'}</p>
            ...
            <button className="btn" onClick={handleSend} disabled={!selectedId || sending}
              style={{ marginTop: 16, fontSize: 17, background: selectedId ? (mode === 'heart' ? 'linear-gradient(135deg,#ff6b6b,#ee4444)' : 'linear-gradient(135deg,#f59e0b,#d97706)') : 'var(--card)', color: '#fff', boxShadow: selectedId ? `0 8px 24px rgba(${mode === 'heart' ? '255,107,107' : '245,158,11'},0.4)` : 'none', opacity: !selectedId ? 0.4 : 1 }}>
        ...
        {mode === 'star' && (
          <div className="animate-fade-in" style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
            <button onClick={() => { setMode('select'); setStarValue(0) }}
              style={{ background: 'none', border: 'none', color: 'var(--muted2)', fontSize: 14, cursor: 'pointer', marginBottom: 20, textAlign: 'left' }}>← 뒤로</button>
            <p style={{ fontSize: 20, fontWeight: 800, marginBottom: 6 }}>⭐ 지금 분위기 몇 점?</p>
            ...
            {starValue > 0 && <p style={{ textAlign: 'center', fontSize: 16, fontWeight: 700, color: '#fbbf24', marginBottom: 32 }}>{MOOD_LABELS[starValue]}</p>}
            <div style={{ flex: 1 }} />
            <button className="btn" onClick={handleSend} disabled={starValue === 0 || sending}
              style={{ fontSize: 17, background: starValue > 0 ? 'linear-gradient(135deg,#7c5cbf,#6d28d9)' : 'var(--card)', color: '#fff', boxShadow: starValue > 0 ? '0 8px 24px rgba(124,92,191,0.4)' : 'none', opacity: starValue === 0 ? 0.4 : 1 }}>
