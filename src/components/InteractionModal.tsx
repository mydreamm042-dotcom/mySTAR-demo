        ...
        {(mode === 'heart' || mode === 'warning') && (
          <div className="animate-fade-in" style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
            ...
            <p style={{ fontSize: 13, color: 'var(--muted2)', marginBottom: 20 }}>
              {mode === 'heart' ? '하트는 10분마다 보낼 수 있어요' : '익명으로 전달돼요'}
            </p>
              ...
              {others.length === 0 ? (
                ...
                return (
                  <button key={p.id} onClick={() => setSelectedId(p.id)}
                    ...
                    <div style={{ width: 40, height: 40, borderRadius: 12, background: 'var(--card2)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18, fontWeight: 700, color: 'var(--text2)' }}>
                      {p.nickname[0] ?? '?'}
                    </div>
