...
import { getCooldownRemaining, setCooldown } from '@/lib/session'
...
export default function InteractionModal({ participants, myParticipantId, roomId, round, onSend, onClose }: Props) {
  const [mode, setMode] = useState<Mode>('select')
  const [selectedId, setSelectedId] = useState<string | null>(null)
  const [starValue, setStarValue] = useState(0)
  const [sending, setSending] = useState(false)
  const [result, setResult] = useState<{ success: boolean; message: string } | null>(null)
  const [starCd, setStarCd] = useState(0)
  const [warnCd, setWarnCd] = useState(0)

  useEffect(() => {
    const tick = () => {
      setStarCd(getCooldownRemaining(roomId, 'star'))
      setWarnCd(getCooldownRemaining(roomId, 'warning'))
    }
    tick()
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (res?.error) {
        ...
        if (mode === 'star') setCooldown(roomId, 'star', 10 * 60 * 1000)
        if (mode === 'warning') setCooldown(roomId, 'warning', 10 * 60 * 1000)
  ...
  const actions = [
    ...
    },
    {
      m: 'warning' as Mode,
      emoji: '🤫', label: '자제 시그널',
      desc: warnCd > 0 ? `${fmtMs(warnCd)} 후 가능` : '익명 · 10분 쿨다운',
      cd: warnCd,
    ...
    },
    {
      m: 'star' as Mode,
      emoji: '⭐', label: '분위기 별점',
      desc: starCd > 0 ? `${fmtMs(starCd)} 후 가능` : '전체 평균 실시간 반영 · 10분 쿨다운',
      cd: starCd,
      color: 'var(--purple-light)', bg: 'rgba(124,92,191,0.1)', border: 'rgba(124,92,191,0.25)',
    },
  ]
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', padding: '56px 24px 24px', overflow: 'hidden' }}>
        ...
        {mode === 'select' && (
          <div className="animate-fade-in" style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {actions.map(({ m, emoji, label, desc, cd, color, bg, border }) => (
              <button key={m} onClick={() => cd === 0 && setMode(m)} disabled={cd > 0}
                style={{
                  display: 'flex', alignItems: 'center', gap: 16,
                  padding: '18px 20px', borderRadius: 18,
                  background: cd > 0 ? 'var(--card)' : bg,
                  border: `1.5px solid ${cd > 0 ? 'var(--border)' : border}`,
                  cursor: cd > 0 ? 'not-allowed' : 'pointer',
                  textAlign: 'left', opacity: cd > 0 ? 0.5 : 1,
                }}>
                <span style={{ fontSize: 36 }}>{emoji}</span>
                <div>
                  <div style={{ fontSize: 17, fontWeight: 700, color: cd > 0 ? 'var(--muted2)' : color, marginBottom: 3 }}>{label}</div>
        ...
        {(mode === 'heart' || mode === 'warning') && (
          <div className="animate-fade-in" style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
            ...
            <p style={{ fontSize: 20, fontWeight: 800, marginBottom: 4 }}>
              {mode === 'heart' ? '💖 호감 표현할 사람' : '🤫 자제 시그널 보낼 사람'}
            </p>
            <p style={{ fontSize: 13, color: 'var(--muted2)', marginBottom: 20 }}>익명으로 전달 · 나만 볼 수 있어요</p>
            ...
            <button className="btn" onClick={handleSend} disabled={!selectedId || sending}
              ...
              {sending ? '전송 중...' : mode === 'heart' ? '💖 호감 시그널 보내기' : '🤫 자제 시그널 보내기'}
            </button>
          </div>
        )}
