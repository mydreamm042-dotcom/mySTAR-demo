              ...
              <div style={{ display: 'flex', alignItems: 'flex-end', gap: 4, height: 100, minWidth: hotBuckets.length * 36 }}>
                {hotBuckets.map((b, i) => (
                  <div key={i} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', flex: 1, height: '100%', justifyContent: 'flex-end' }}>
                    ...
                    <div style={{ width: '100%', minWidth: 20, height: Math.max(4, Math.round((b.count / maxHot) * 72)), borderRadius: '4px 4px 0 0', background: b.count > 0 && b.count === maxHot ? 'linear-gradient(180deg,#f97316,#ef4444)' : b.count > 0 ? 'rgba(249,115,22,0.45)' : 'rgba(255,255,255,0.06)' }} />
        ...
        <div className="card" style={{ padding: 20 }}>
          ...
          {heartTop3.length === 0 ? (
            ...
            <div key={p!.id} style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 10 }}>
              ...
              <p style={{ flex: 1, fontSize: 14, fontWeight: 700 }}>{p!.id === roomData.participantId ? roomData.nickname + ' (나)' : p!.nickname}</p>
