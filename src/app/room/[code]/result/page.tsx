...
export default function ResultPage({ params }: { params: Promise<{ code: string }> }) {
  ...
  const [resolvedRoomId, setResolvedRoomId] = useState<string | null>(roomData?.roomId ?? null)
  const [resolvedRoomName, setResolvedRoomName] = useState(roomData?.roomName ?? '')
  const [data, setData] = useState<ResultData | null>(null)
  const [voted, setVoted] = useState<string | null>(null)
  const [pendingVote, setPendingVote] = useState<string | null>(null)
  const [votingSending, setVotingSending] = useState(false)

  const fetchResult = async (rid: string) => {
    const [rRes, rxRes, nRes, evRes] = await Promise.all([
      fetch('/api/rooms/' + code),
      fetch('/api/reactions?room_id=' + rid),
      fetch('/api/notification-rounds?room_id=' + rid),
      fetch('/api/end-votes?room_id=' + rid),
    ])
  ...
  useEffect(() => {
    if (roomData) {
      fetchResult(roomData.roomId)
    } else {
      fetch('/api/rooms/' + code)
        .then(r => r.json())
        .then(d => {
          if (d.room) {
            setResolvedRoomId(d.room.id)
            setResolvedRoomName(d.room.name)
            fetchResult(d.room.id)
          } else {
            router.replace('/')
          }
        })
  ...
  const handleVote = async (participantId: string) => {
    if (voted || votingSending || !roomData || !resolvedRoomId) return
    setVotingSending(true)
    const res = await fetch('/api/end-votes', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ room_id: resolvedRoomId, voter_session: getSessionToken(), voted_for_id: participantId }),
    })
    if (res.ok) {
      setVoted(participantId)
      setPendingVote(null)
      fetchResult(resolvedRoomId)
    }
  ...
  const displayName = roomData?.roomName ?? resolvedRoomName
  ...
  const others = data.participants.filter(p => p.id !== myId)
  ...
  const canVote = !!roomData && others.length > 0
  ...
  return (
    <main style={{ display: 'flex', flexDirection: 'column', minHeight: '100dvh', paddingBottom: 40 }}>
      <div style={{ padding: '52px 20px 24px', textAlign: 'center', background: 'linear-gradient(180deg, rgba(255,107,107,0.08) 0%, transparent 100%)' }} className="animate-fade-in">
        ...
        <h1 style={{ fontSize: 26, fontWeight: 800, marginBottom: 6 }}>{displayName}</h1>
      ...
      <div style={{ padding: '0 20px', marginBottom: 20 }} className="animate-fade-in">
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10 }}>
          {[
            ...
            { e: '🔥', v: hotIndex + '%', l: 'Hot 지수', c: hotIndex >= 90 ? '#f97316' : '#fbbf24' },
          ].map(s => (
      ...
      <div style={{ padding: '0 20px', display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          {heartTop3.length === 0 ? (
            ...
            <div key={p!.id} style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 10 }}>
              ...
              <p style={{ flex: 1, fontSize: 14, fontWeight: 700 }}>
                {p!.id === myId ? (roomData?.nickname ?? '나') + ' (나)' : '누군가'}
              </p>
        ...
        <div className="card" style={{ padding: 20 }}>
          ...
          <p style={{ fontSize: 12, color: 'var(--muted2)', marginBottom: 16 }}>익명 투표 · 1명 선택 후 투표하기 버튼</p>

          {!canVote ? (
            <p style={{ fontSize: 13, color: 'var(--muted2)', textAlign: 'center', padding: '8px 0' }}>
              {roomData ? '다른 참여자가 없어요' : '투표는 방 참여자만 가능해요'}
            </p>
          ) : others.map(p => {
            ...
            return (
              <button key={p.id}
                onClick={() => !voted && !votingSending && setPendingVote(isPending ? null : p.id)}
                disabled={!!voted || votingSending}
                style={{
                  width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'space-between',
                  padding: '14px 16px', borderRadius: 14, marginBottom: 8,
                  background: isVoted ? 'rgba(16,185,129,0.12)' : isPending ? 'rgba(16,185,129,0.07)' : 'var(--card2)',
                  border: '1.5px solid ' + (isVoted ? 'rgba(16,185,129,0.4)' : isPending ? 'rgba(16,185,129,0.3)' : 'var(--border)'),
                ...
                {isPending && !voted && <span style={{ fontSize: 13, color: '#10b981', fontWeight: 600 }}>선택됨 ✓</span>}
                {voted && (data.endVoteCounts[p.id] ?? 0) > 0 && (
                  <div style={{ background: 'rgba(16,185,129,0.15)', padding: '4px 10px', borderRadius: 20, display: 'flex', alignItems: 'center', gap: 4 }}>
                    <span style={{ fontSize: 14 }}>🙋</span>
                    <span style={{ fontSize: 14, fontWeight: 800, color: '#10b981' }}>{data.endVoteCounts[p.id]}</span>
                  </div>
                )}
              </button>
            )
          ...
          {canVote && !voted && pendingVote && (
            <button onClick={() => handleVote(pendingVote)} disabled={votingSending}
              className="btn"
              style={{
            ...
            <div style={{ marginTop: 16, paddingTop: 16, borderTop: '1px solid var(--border)' }}>
              ...
              {seeAgainTop.map(({ participant: p, count }, i) => (
                <div key={p!.id} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '6px 0', fontSize: 14 }}>
                  <span>{medals[i]}</span>
                  <span style={{ flex: 1 }}>{p!.id === myId ? '나' : '누군가'}</span>
