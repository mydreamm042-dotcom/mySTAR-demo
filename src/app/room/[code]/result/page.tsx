...
export default function ResultPage({ params }: { params: Promise<{ code: string }> }) {
  ...
  const fetchResult = async () => {
    ...
    setData({ participants: rData.participants ?? [], reactions: rxData.reactions ?? [], rounds: nData.rounds ?? [], endVoteCounts: evData.counts ?? {} })
  }
  ...
  data.reactions.filter(r => r.type === 'heart').forEach(r => { heartCounts[r.receiver_id] = (heartCounts[r.receiver_id] ?? 0) + 1 })
  const heartTop3 = Object.entries(heartCounts).sort(([, a], [, b]) => b - a).slice(0, 3)
    .map(([id, count]) => ({ participant: data.participants.find(p => p.id === id), count })).filter(x => x.participant)
  ...
  const seeAgainTop = Object.entries(data.endVoteCounts).sort(([, a], [, b]) => b - a).slice(0, 5)
    .map(([id, count]) => ({ participant: data.participants.find(p => p.id === id), count })).filter(x => x.participant)
    ...
    <main style={{ display: 'flex', flexDirection: 'column', minHeight: '100dvh', paddingBottom: 40 }}>
      ...
      <div style={{ padding: '0 20px', marginBottom: 20 }}>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10 }}>
          {[{ e: '👥', v: data.participants.length, l: '참여자', c: 'var(--text)' }, { e: '💖', v: totalHearts, l: '하트', c: '#ff6b6b' }, { e: '⭐', v: avgMood !== null ? avgMood.toFixed(1) : '-', l: '평균 분위기', c: '#fbbf24' }].map(s => (
      ...
      <div style={{ padding: '0 20px', display: 'flex', flexDirection: 'column', gap: 16 }}>
        <div className="card" style={{ padding: 20 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 16 }}><span style={{ fontSize: 20 }}>💖</span><h2 style={{ fontSize: 15, fontWeight: 800, color: '#ff6b6b' }}>오늘의 하트 Top 3</h2></div>
          {heartTop3.length === 0 ? <p style={{ fontSize: 14, textAlign: 'center', padding: '16px 0', color: 'var(--muted2)' }}>아직 하트가 없어요</p>
            ...
            <div key={p!.id} style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 10 }}>
              ...
              <p style={{ flex: 1, fontSize: 14, fontWeight: 700 }}>{p!.id === roomData.participantId ? roomData.nickname + ' (나)' : '누군가'}</p>
              <div style={{ background: 'rgba(255,107,107,0.12)', padding: '4px 12px', borderRadius: 20, display: 'flex', alignItems: 'center', gap: 4 }}>
                <span style={{ fontSize: 14 }}>💖</span><span style={{ fontSize: 14, fontWeight: 800, color: '#ff6b6b' }}>{count}</span>
              </div>
            </div>
          ))}
        </div>
        <div className="card" style={{ padding: 20 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 16 }}><span style={{ fontSize: 20 }}>⭐</span><h2 style={{ fontSize: 15, fontWeight: 800, color: '#fbbf24' }}>분위기 타임라인</h2></div>
          {moodTimeline.length === 0 ? <p style={{ fontSize: 14, textAlign: 'center', padding: '16px 0', color: 'var(--muted2)' }}>투표 기록이 없어요</p>
            ...
            <div key={round.id} style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 }}>
              ...
              <div style={{ flex: 1, height: 8, borderRadius: 4, background: 'rgba(255,255,255,0.06)', overflow: 'hidden' }}>
                {round.avg !== null && <div style={{ height: '100%', borderRadius: 4, width: (round.avg / 5 * 100) + '%', background: 'linear-gradient(90deg, #f59e0b, #fbbf24)' }} />}
              </div>
              <span style={{ fontSize: 14, fontWeight: 800, width: 36, textAlign: 'right', color: '#fbbf24' }}>{round.avg !== null ? round.avg.toFixed(1) : '-'}</span>
            </div>
          ))}
        </div>
        <div className="card" style={{ padding: 20 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}><span style={{ fontSize: 20 }}>🙋</span><h2 style={{ fontSize: 15, fontWeight: 800, color: '#10b981' }}>다음에 또 보고 싶은 사람</h2></div>
          <p style={{ fontSize: 12, color: 'var(--muted2)', marginBottom: 16 }}>익명 투표 · 1명 선택</p>
          {others.length === 0 ? <p style={{ fontSize: 14, textAlign: 'center', padding: '16px 0', color: 'var(--muted2)' }}>다른 참여자가 없어요</p>
            : others.map(p => {
              ...
              <button key={p.id} onClick={() => handleVote(p.id)} disabled={!!voted || votingSending}
                ...
                {voted && voteCount > 0 && (
                  <div style={{ background: 'rgba(16,185,129,0.15)', padding: '4px 10px', borderRadius: 20, display: 'flex', alignItems: 'center', gap: 4 }}>
                    <span style={{ fontSize: 14 }}>🙋</span><span style={{ fontSize: 14, fontWeight: 800, color: '#10b981' }}>{voteCount}</span>
                  </div>
                )}
              </button>
            )
